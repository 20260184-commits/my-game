...
if hasattr(sys, '_MEIPASS'):
    base = sys._MEIPASS # 빌드 후: 임시 폴더
else:
    base = os.path.dirname(__file__) # 개발 중: .py 파일 위치
...
try:
    UI_CLICK_SOUND = pygame.mixer.Sound(resource_path(UI_CLICK_SFX_FILE))
except:
    UI_CLICK_SOUND = None
...
def play_game(settings):
    try: pygame.mixer.music.load(resource_path(MUSIC_FILE)); pygame.mixer.music.set_volume(settings.bgm_volume); pygame.mixer.music.play(-1)
    except: pass
    ...
    try:
        bg_img = pygame.image.load(resource_path(BG_FILE)).convert_alpha()
        bg_img = pygame.transform.scale(bg_img, (WIDTH, HEIGHT)) # 화면 크기에 맞게 조절
    except:
        bg_img = None # 이미지 로드 실패 시 대비
        print("배경 이미지를 찾을 수 없습니다.")
    try:
        boss_img = pygame.image.load(resource_path(BOSS_IMG_FILE)).convert_alpha()
        boss_img = pygame.transform.scale(boss_img, (BOSS_SIZE, BOSS_SIZE))

        parry_sound = pygame.mixer.Sound(resource_path(PARRY_SFX_FILE))
        parry_sound.set_volume(settings.sfx_volume) # 설정 메뉴의 SFX 볼륨 적용

        miss_sound = pygame.mixer.Sound(resource_path(MISS_SFX_FILE))
        miss_sound.set_volume(settings.sfx_volume)

        damage_sound = pygame.mixer.Sound(resource_path(DAMAGE_SFX_FILE))
