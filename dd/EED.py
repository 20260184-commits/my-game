...
import gc    # 🌟 [신규 추가] 1% Low 프레임 드랍을 소거해 주는 파이썬 핵심 가비지 컬렉터 모듈
...
class ComboDisplay:
    def __init__(self):
        ...
        self.active = False
        self.combo_count = 0
        # 🌟 [1% LOW FPS 극강 최적화] CPU 스케일링 부하를 완벽 소거하기 위해 사전 선행 렌더링 스케일 캐시 저장소 구축
        self.text_cache = {}
        self.shadow_cache = {}
    ...
    def trigger(self, count):
        ...
        base_text_surf = self.font.render(f"{self.combo_count} HIT!", True, (255, 200, 0))
        base_shadow_surf = self.font.render(f"{self.combo_count} HIT!", True, (0, 0, 0))
        
        # 🌟 [1% LOW FPS 최적화] 루프 도중 transform.scale()을 호출하는 무거운 연산들을 단 1프레임 동안 미리 연산하여 VRAM 영역으로 캐싱합니다.
        self.text_cache.clear()
        self.shadow_cache.clear()
        w, h = base_text_surf.get_size()
        for t in range(21):
            scale = 1.0 + (math.sin((t / 20) * 3.14) * 0.5)
            scaled_w, scaled_h = int(w * scale), int(h * scale)
            self.text_cache[t] = pygame.transform.scale(base_text_surf, (scaled_w, scaled_h))
            self.shadow_cache[t] = pygame.transform.scale(base_shadow_surf, (scaled_w, scaled_h))
    ...
    def draw(self, surface, player_rect):
        ...
        # 🌟 [1% LOW FPS 최적화] 매 프레임 발생하던 스케일 보정을 단순 인덱스 딕셔너리 룩업(O(1) 속도)으로 교체합니다.
        text_surf = self.text_cache.get(self.timer, self.text_cache[0])
        shadow_surf = self.shadow_cache.get(self.timer, self.shadow_cache[0])
        scaled_h = text_surf.get_height()
...
def load_sprite_sheet(filename, frame_count):
    """🌟 [최적화 완료] 기존 캐릭터 및 적군 클래스에서 정상 참조하는 CPU 전용 초고속 스프라이트 시트 분할 로더입니다."""
    try:
...
class Entity(pygame.sprite.Sprite):
    ...
    def take_damage(self, amount, attacker, attack_type): # attack_type 인자 추가
        ...
        if is_guarding:
            ...
            # 🌟 [버그 수정] 가드에 실패하고 피격당한 모든 상황(강공격 포함)에서 블로킹 상태를 확실히 False로 초기화합니다.
            # 기존에는 강공격(HEAVY) 피격 시 이 변수가 False로 갱신되지 않고 이전 가드 상태(True)로 남아 캔슬 대쉬 경직 연출이 스킵되던 버그를 예방합니다.
    ...
    def update_ai(self, target, active_explosions):
        ...
        if self.is_boss:
            ...
            if self.is_transforming:
                ...
                self.vel_x = 0
                # 🌟 [보스 경직 무시 사기 행위 차단] 
                # 보스가 기를 모으는 도중이라도, 플레이어가 콤보 캔슬 대쉬로 시공간 정지(스턴)를 건 상황(cancel_dash_stun_timer > 0)이라면 스스로 기절을 해제하는 꼼수를 차단합니다.
                if self.state == "HIT" and getattr(self, "cancel_dash_stun_timer", 0) <= 0:
                    self.state = "IDLE" 
...
def main():
    ...
    # 🌟 [1% Low 프레임 극대화] 난타전 속에서 무지성으로 가비지 수거 연산이 돌아 화면이 미세하게 정지하는 렉을 막기 위해 전투 중 오토 GC를 차단합니다.
    gc.disable()
    ...
    # 🌟 [하드웨어 비디오 버퍼 가속 적용]
    # 모니터 주사율과 렌더링 박자를 맞추어 화면 찢어짐(Tearing)을 막고, SDL2 드라이버의 그래픽 버퍼 가속을 타도록 DOUBLEBUF 설정을 인가합니다.
    ...
    def draw_loading_screen(progress, current_task):
        ...
        # 선행 변환 완료된 screen 버퍼를 화면에 갱신합니다.
        pygame.display.flip()
        
        # 🌟 [1% LOW FPS 극강 최적화] 로딩 진행 중 쌓였던 메모리 가비지 부산물들을 수동으로 대량 강제 수거 소거합니다.
        gc.collect()
    ...
    while running:
        ...
        # 🌟 [신규 추가] 일시정지(PAUSE) 상태 UI 및 조작 로직
            ...
            def execute_reset_and_return():
                # 🌟 [전역 스코프 교정] transition_state, transition_x는 모듈 수준 글로벌 변수이므로 global 식별자 명단으로 재분류합니다.
                global GAME_STATE, KO_CINEMATIC_TIMER, KO_TRIGGERED, current_stage_idx, stage_info, match_timer, time_over, countdown_timer, death_delay_timer, game_over_alpha, game_cleared, game_clear_alpha, post_clear_timer, post_death_timer, CAMERA_X, transition_state, transition_x
                # 🌟 [비지역 바인딩 교정] 오직 main() 내부의 지역 인스턴스인 enemy만 nonlocal 대상으로 등록하여 크래시를 해결합니다.
                nonlocal enemy
                
                GAME_STATE = "MENU"
                
                # 🌟 [1% LOW FPS 극강 최적화] 메인 메뉴 복귀에 앞서 인게임 파편들을 수동 수거 제거합니다.
                gc.collect()
        ...
        if hitstop_timer > 0:
            ...
            if player.state == "DASH" and player.is_cancel_dash:
                # 🌟 [보스 경직 씹힘 버그 해결] 보스는 약공격 피격 시 슈퍼아머로 인해 HIT이 아닌 IDLE 상태를 유지해 경직을 우회합니다.
                # 상대가 기절 상태(HIT)이거나 보스 캐릭터인 경우 강제로 HIT(경직) 상태로 전이시키고 스턴을 적용합니다.
                is_enemy_eligible_for_stun = (enemy.state == "HIT" or enemy.is_boss) and not enemy.is_blocking
                if is_enemy_eligible_for_stun:
                    enemy.state = "HIT" # 보스의 슈퍼아머 강제 해제 및 경직 전이
                    enemy.timer = 0
                    enemy.hit_stun_timer += DASH_CANCEL_STUN_BONUS
                    enemy.cancel_dash_stun_timer = enemy.hit_stun_timer
                    # 플레이어 위치에 기류 저항 돌풍 스폰
            ...
            if enemy.state == "DASH" and enemy.is_cancel_dash:
                # 플레이어가 보스의 대쉬 캔슬 스턴 사정권에 들어온 경우의 대칭 처리
                is_player_eligible_for_stun = (player.state == "HIT" or player.is_boss) and not player.is_blocking
                if is_player_eligible_for_stun:
                    player.state = "HIT"
                    player.timer = 0
                    player.hit_stun_timer += DASH_CANCEL_STUN_BONUS
                    player.cancel_dash_stun_timer = player.hit_stun_timer
                    # AI 위치에 기류 저항 돌풍 스폰
            ...
            if transition_state == "WIPE_IN":
                ...
                # --- [장막이 화면을 채웠을 때 실행] 다음 스테이지 데이터 로드 ---
                    ...
                    # 🌟 [1% LOW FPS 극강 최적화] 무대 장막이 완전히 가려져서 보이지 않는 찰나의 프레임 동안 
                    # 대대적인 가비지 컬렉션을 실행하여 다음 무대 전투 시 끊김을 소거합니다.
                    gc.collect()
